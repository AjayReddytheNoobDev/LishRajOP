...
async def addchannel(client, message):
    ...
    try:
        ...
        await message.reply_text(
            f"✅ **userbot already entered chat** Aankhe khol yaar",
        )
    except Exception as e:
        print(e)
        await message.reply_text(
            f"Sedddd, Flood Wait Error \n\n**userbot couldn't join your group due to heavy join requests for userbot** Yup my boi is so famous"
            "\n\n**or add assistant manually to your Group and try again**",
        )
        return
    await message.reply_text(
        f"✅ **userbot succesfully entered chat** aagya mera lob❤",
    )
...
async def rem(client, message):
    try:
        await USER.send_message(message.chat.id, "✅ userbot successfully left chat. Nikal diya na, abb me bhi kya karu?? muje bhi nikaldo. dhokebaazi duniya")
