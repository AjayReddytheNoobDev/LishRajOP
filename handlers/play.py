...
async def settings(client, message):
    ...
    if stats:
        ...
        await message.reply(
            "Ff **voice chat not found**\n\n» please turn on the voice chat first kid"
        )
...
async def play(_, message: Message):
        ...
        for administrator in administrators:
            if administrator == message.from_user.id:
                if message.chat.title.startswith("Channel Music: "):
                    await lel.edit(
                        f"LMAO, Please add the userbot to your channel first.**",
                    )
                try:
                    invitelink = await _.export_chat_invite_link(chid)
                except:
                    await lel.edit(
                        "**To use me, I need to be an Administrator** with the all these phuking rights:\n\n» ❌ __Delete messages__\n» ❌ __Ban users__\n» ❌ __Add users__\n» ❌ __Manage voice chat__\n\n**Then type /reload**",
                    )
                    return
                try:
                    ...
                    # print(e)
                    await lel.edit(
                        f"Fffff **Flood Wait Error** \n\n**userbot can't join this group due to many join requests for userbot. yes my boi is famous**"
    ...
    if audio:
        ...
        title = file_name
        thumb_name = "https://telegra.ph/file/eb2f5ca34b47b1ad289e7.jpg"
        ...
        query = toxt
        await lel.edit("🔎 **searching... kanta laga oima oima oima**")
        ...
        try:
            ...
            await lel.edit(
                "Seddddd, **couldn't find song you requested**\n\n» **please provide the correct song name or include the artist's name as well. Dhang se likhlo bc**"
            )
            print(str(e))
            return
        keyboard = InlineKeyboardMarkup(
        ...
        try:
            ...
            await lel.edit(
                "SEDDDD, **Song name not detected**\n\n» **please provide the name of the song you want to play** Dhang se likhlo bc"
            )
        # veez project
        try:
            toxxt = "\n"
            ...
            while j < 5:
                ...
                toxxt += f" └  __Powered by @SERPENT_BOTS__\n\n"
                j += 1
            keyboard = InlineKeyboardMarkup(
                [
                    [
            ...
            try:
                ...
                await lel.edit(
                    "Sedddd, **Couldn't find song you requested**\n\n» **please provide the correct song name or include the artist's name as well** Dhang se likhlo bc"
                )
                print(str(e))
                return
            keyboard = InlineKeyboardMarkup(
...
async def lol_cb(b, cb):
    ...
    try:
        ...
        if (dur / 60) > DURATION_LIMIT:
            await cb.message.edit(
                f"❌ **Music with duration more than** `{DURATION_LIMIT}` **minutes, can't play !** Song play karu ya Movie, kaunse kaunse log use kar rha hai"
            )
            return
    except:
        pass
    ...
    keyboard = InlineKeyboardMarkup(
        [
            ...
            [InlineKeyboardButton("• CHANNEL", url=f"https://t.me/SERPENT_BOTS")],
        ]
    )
    await generate_cover(title, thumbnail, ctitle)
    file_path = await converter.convert(youtube.download(url))
...
async def ytplay(_, message: Message):
        ...
        for administrator in administrators:
            if administrator == message.from_user.id:
                ...
                try:
                    ...
                    await lel.edit(
                        "**To use me, I need to be an Administrator with all these phuking rights:\n\n» ❌ __Delete messages__\n» ❌ __Ban users__\n» ❌ __Add users__\n» ❌ __Manage voice chat__\n\n**Then type /reload**",
                    )
                    return
                ...
                try:
                    await USER.join_chat(invitelink)
                    await lel.edit(
                        f"✅ **userbot succesfully entered chat** aagya mera bc",
                    )
                    ...
                    # print(e)
                    await lel.edit(
                        f"Fffff **Flood Wait Error** \n\n**userbot can't join this group due to many join requests for userbot.** Yup, my boi is very famous"
                        f"\n\n**or add @{ASSISTANT_NAME} to this group manually then try again.**",
                    )
    try:
        await USER.get_chat(chid)
    ...
    if chat_id in callsmusic.pytgcalls.active_calls:
        ...
        try:
            ...
            await lel.edit(
                "F Nashedi **voice chat not found**\n\n» please turn on the voice chat first kid"
            )
            return
        await lel.delete()
        await message.reply_photo(
