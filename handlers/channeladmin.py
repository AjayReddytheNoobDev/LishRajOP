...
async def update_admin(client, message):
    ...
    await message.reply_text(
        "✅ Bot **reloaded perfectly !**\n✅ **Admin list** is **updated ! comgrats**"
    )
...
async def channel_pause(_, message: Message):
    try:
        ...
        await message.reply("❌ `NOT_LINKED`\n\n• **The userbot could not play music, due to group not linked to channel yet.** Bery Seddd Bro")
        return
    chat_id = chid
...
async def channel_resume(_, message: Message):
    try:
        ...
        await message.reply("❌ `NOT_LINKED`\n\n• **The userbot could not play music, due to group not linked to channel yet.** Bery Sedd Bro")
        return
    chat_id = chid
    if (chat_id not in callsmusic.pytgcalls.active_calls) or (
...
async def skip(_, message: Message):
    ...
    if not qeue:
        return
    await message.reply_text("⏭ **You've skipped to the next song.** Thank you for saving me")
